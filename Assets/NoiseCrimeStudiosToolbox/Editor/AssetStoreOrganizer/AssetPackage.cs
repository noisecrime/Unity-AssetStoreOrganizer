...
{
    /// <summary>AssetStorePackage Container</summary>
    /// <remarks>Editor\Mono\AssetStore\AssetStoreContext.cs</remarks>
    public class AssetPackage
	{
        ...
        private static Regex            s_StandardPackage52RegExp   = new Regex("/Standard Packages/(Character\\ Controller|Glass\\ Refraction\\ \\(Pro\\ Only\\)|Image\\ Effects\\ \\(Pro\\ Only\\)|Light\\ Cookies|Light\\ Flares|Particles|Physic\\ Materials|Projectors|Scripts|Standard\\ Assets\\ \\(Mobile\\)|Skyboxes|Terrain\\ Assets|Toon\\ Shading|Tree\\ Creator|Water\\ \\(Basic\\)|Water\\ \\(Pro\\ Only\\))\\.unitypackage$", RegexOptions.IgnoreCase);
	//	private static Regex        	s_StandardPackage54RegExp   = new Regex("/Standard Packages/(2D\\  Assets|Toon\\ Shading|Tree\\ Creator|Water\\ \\(Basic\\)|Water\\ \\(Pro\\ Only\\))\\.unitypackage$", RegexOptions.IgnoreCase);
	...
	//	private static Regex        	s_InvalidPathCharsRegExp	= new Regex("[^a-zA-Z0-9() _-]");
		...
		public static long CreatePackageInfoFromLocalStorage( string fullFilePath, List<AssetPackage> results )
		{
			...
			try
			{
				...
				using ( BinaryReader reader = new BinaryReader( new FileStream( fullFilePath, FileMode.Open ) ) )
				{
					...
					{
						Debug.LogErrorFormat( "CreatePackageInfoFromLocalStorage Error: Invalid Package\n{0}\nBad magicNum: {1}", fullFilePath, magicNum );
					}
				}
		...
		//	bool isUnityStandardAsset	= IsBuiltinStandardAsset( fullFilePath );
			...
			if ( string.IsNullOrEmpty( pack.unity_version ) ) pack.unity_version = "NA";  // Prior to Unity 5.x this was empty
		...
		private static AssetPackage BuildDefaultStorePackage( string title, string unityVersion, string version, string category, string categoryID, string publisher, string publisherID )
		{
			...
			uasp.link				= new StorePackageLink();
			uasp.category			= new StorePackageLabelWithID();
			uasp.publisher			= new StorePackageLabelWithID();

			uasp.unity_version		= unityVersion;
			uasp.pubdate			= "NA";
			uasp.version			= version;
			uasp.upload_id			= -1;
			uasp.version_id			= -1;
			uasp.id					= -1;
			uasp.title				= title;
